...
(rm.16 <- c(1,5,16))
# I didn't think we sampled all the sites in 2016 but ok
...
#2-22-2018#write.csv(cpeSum,"Data/Clean-Data/summary-data/cpeSum.csv",row.names = FALSE)
```


## CPUE by Gabelhouse Length Category
  
1) First, I will make new data object (bio2) and add species names (Sp.Names) character variable. This way I can conviniently sort each species of fish into its correct Gabelhouse length category based on its length.  
...
#### Make New Biodat with Gabelhouse Length Categories Assigned to Species ####   
...
#### Add Zeroes for Each Species and Gabelhouse Length Category ####

```{r Add zeros for species by site, tidy=TRUE}

#cpe$ID <- paste(cpe$Year,".",cpe$Site)

cpe %<>% addZeroCatch("Site","Species", zerovar = "caught") %>%
#cpe %<>% addZeroCatch("ID","Species", zerovar = "caught") %>%
...
headtail(cpe)
str(cpe)
#2-9-2018#write.csv(cpe,"Data/Clean-Data/CPUE-gcat_2013-2016.csv",row.names = FALSE)
```
...
#2-9-2018#write.csv(cpeSum.gcat,"Data/Clean-Data/summary-data/cpeSum_gcat.csv",row.names = FALSE)
```
```{r, echo=FALSE}
...
sum.disp <- c(sum.disp[,c(1:6)],round(sum.disp[,c(6:9)],3)) %>% 
  as.data.frame()
