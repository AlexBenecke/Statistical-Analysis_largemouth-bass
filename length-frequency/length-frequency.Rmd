...
#1-10-2018#write.csv(lmb.17, file = "Data/Clean-Data/minor-data/lmb.17.csv")
```
\newpage
...
### length frequency all years 2013 - 2017
...
```
```{r Make Adjuster p-values a little easier to read, cache=TRUE, tidy=TRUE, echo = FALSE}
...
```
```{r Lets look at results from the K-S test, cache=FALSE, tidy=TRUE}
...
### Length Frequency 2013 - 2016 (without 2017)
...
```{r ECDF 2, cache=TRUE, tidy=TRUE, echo=FALSE}
par(mfrow=c(1,1)) ### reset to default
...
```{r Kolmogorov-Smirnov Test 2, cache=TRUE, tidy=TRUE, include=FALSE}
### 2013 and 2014
(ks.13.14 <- ks.test(lmb.13$Length,lmb.14$Length))
# Warning message:
# In ks.test(lmb.14$Length, lmb.15$Length) :
...
(p.val <- p.adjust(ks.ps))
```
```{r Make Adjuster p-values a little easier to read 2, cache=TRUE, tidy=TRUE, echo = FALSE}
(D <- c(ks.13.14$statistic[[1]], 
        ks.13.15$statistic[[1]],
        ks.13.16$statistic[[1]],
        ks.14.15$statistic[[1]], 
...
```
```{r Lets look at results from the K-S test 2, cache=FALSE, tidy=TRUE}
(p.yr <- data.frame(yrs,D,p.val))
```
...
### Length Frequency 2013 - 2016 (No Fish < 200mm)
...
lmbs.17 <- filter(lmb.s,Year==2017)
```
...
### 2017
(Freq17s.20 <- xtabs(~lcat20,data=lmbs.17))
prop.table(Freq17s.20)*100 ## Peak at 340mm
```
...
### Compare Length Frequency Between Years (for stock only)
#### Kolmogorov-Smirnov Test (Stock)
...
(yrs <- c("13-14","13-15","13-16","14-15","14-16","15-16"))
```
```{r Lets look at results from the K-S test Stock, cache=FALSE, tidy=TRUE}
